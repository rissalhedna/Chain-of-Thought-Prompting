...
class CoTDataModule:
    ...
    processors: dict[str, Callable[[pd.DataFrame], None]] = field(
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.dataset_name == "openai/gsm8k":
                ...
                dataset_dict = load_dataset(
                    self.dataset_name, cache_dir=self.cache_dir, split=self.split
                )
        ...
        if self.num_samples:
            if self.num_samples > len(self.data):
                raise ValueError(
                    f"Requested {self.num_samples} samples, but the dataset only has {len(self.data)} samples."
                )
            self.data = self.data.sample(
                n=self.num_samples, random_state=42
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, int):
            ...
            raise TypeError(
                f"Invalid argument type: {type(index)}. Expected int or slice."
            )
